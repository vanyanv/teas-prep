/**
 * Blind verification for wave-2 questions (scratchpad/gen2-*.json).
 *  --blind : write blind2-<file>.json (ref/type/stem/options, NO key) + keys2.json
 *  --diff  : compare verdict2-*.json ([{ref, answer}]) to keys2.json
...
const files = () =>
  readdirSync(SCRATCH).filter((f) => f.startsWith("gen2-") && f.endsWith(".json")).sort();
...
if (process.argv.includes("--blind")) {
  ...
  for (const file of files()) {
    ...
    const topic = file.replace(/^gen2-/, "").replace(/\.json$/, "");
    ...
    writeFileSync(join(SCRATCH, `blind2-${topic}.json`), JSON.stringify(blind, null, 2));
  }
  writeFileSync(join(SCRATCH, "keys2.json"), JSON.stringify(keys));
  console.log(`Blinded ${Object.keys(keys).length} wave-2 items across ${files().length} files.`);
}

if (process.argv.includes("--diff")) {
  const keys = JSON.parse(readFileSync(join(SCRATCH, "keys2.json"), "utf8")) as Record<
    string,
    { type: string; correct: unknown }
  >;
  const verdicts: Record<string, unknown> = {};
  for (const f of readdirSync(SCRATCH).filter((f) => f.startsWith("verdict2-") && f.endsWith(".json")))
