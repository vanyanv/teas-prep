/**
 * Blind verification for wave-3 questions (scratchpad/gen3-*.json).
 *  --blind : write blind3-<file>.json (ref/type/stem/options, NO key) + keys3.json
 *  --diff  : compare verdict3-*.json ([{ref, answer}]) to keys3.json
...
const files = () =>
  readdirSync(SCRATCH).filter((f) => f.startsWith("gen3-") && f.endsWith(".json")).sort();
...
if (process.argv.includes("--blind")) {
  ...
  for (const file of files()) {
    ...
    const topic = file.replace(/^gen3-/, "").replace(/\.json$/, "");
    ...
    writeFileSync(join(SCRATCH, `blind3-${topic}.json`), JSON.stringify(blind, null, 2));
  }
  writeFileSync(join(SCRATCH, "keys3.json"), JSON.stringify(keys));
  console.log(`Blinded ${Object.keys(keys).length} wave-3 items across ${files().length} files.`);
}

if (process.argv.includes("--diff")) {
  const keys = JSON.parse(readFileSync(join(SCRATCH, "keys3.json"), "utf8")) as Record<
    string,
    { type: string; correct: unknown }
  >;
  const verdicts: Record<string, unknown> = {};
  for (const f of readdirSync(SCRATCH).filter((f) => f.startsWith("verdict3-") && f.endsWith(".json")))
