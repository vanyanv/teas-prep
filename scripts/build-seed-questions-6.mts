/**
 * Wave-6 question integration. Reads scratchpad/gen6-*.json, validates, drops
 * any stem that duplicates an existing question (the committed bank) or another
 * wave-6 item, shuffles options, and emits src/content/questions.generated.6.ts
 * as GENERATED_QUESTIONS_6 (additive — leaves questions.generated.ts untouched).
 *
...
const OUT = "src/content/questions.generated.6.ts";
...
for (const file of readdirSync(SCRATCH).filter((f) => f.startsWith("gen6-") && f.endsWith(".json")).sort()) {
  let parsed: unknown;
  try {
...
console.log(`Wave-6 new items: ${all.length}  (dropped ${dropped} duplicates)`);
...
if (process.argv.includes("--write")) {
  ...
  const ts = `import type { SeedQuestion } from "./seed-types";\n\n// AUTO-GENERATED by scripts/build-seed-questions-2.mts. Do not edit by hand.\n// Wave 6: more original questions per skill, independently blind-verified.\nexport const GENERATED_QUESTIONS_6: SeedQuestion[] = [\n${body}\n];\n`;
  writeFileSync(OUT, ts);
  console.log(`\nWrote ${all.length} wave-6 questions to ${OUT}`);
}
