/**
 * Wave-7 question integration. Reads scratchpad/gen7-*.json, validates, drops
 * any stem that duplicates an existing question (the committed bank) or another
 * wave-7 item, shuffles options, and emits src/content/questions.generated.7.ts
 * as GENERATED_QUESTIONS_7 (additive — leaves questions.generated.ts untouched).
 *
 * Usage: tsx scripts/build-seed-questions-7.mts [--write]
 */
...
const OUT = "src/content/questions.generated.7.ts";
...
for (const file of readdirSync(SCRATCH).filter((f) => f.startsWith("gen7-") && f.endsWith(".json")).sort()) {
  let parsed: unknown;
  try {
...
console.log(`Wave-7 new items: ${all.length}  (dropped ${dropped} duplicates)`);
...
if (process.argv.includes("--write")) {
  ...
  const ts = `import type { SeedQuestion } from "./seed-types";\n\n// AUTO-GENERATED by scripts/build-seed-questions-7.mts. Do not edit by hand.\n// Wave 7: more original questions per skill, independently blind-verified.\nexport const GENERATED_QUESTIONS_7: SeedQuestion[] = [\n${body}\n];\n`;
  writeFileSync(OUT, ts);
  console.log(`\nWrote ${all.length} wave-7 questions to ${OUT}`);
}
