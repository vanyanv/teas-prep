/**
 * Wave-3 question integration. Reads scratchpad/gen3-*.json, validates, drops
 * any stem that duplicates an existing question (the committed bank) or another
 * wave-3 item, shuffles options, and emits src/content/questions.generated.3.ts
 * as GENERATED_QUESTIONS_3 (additive — leaves questions.generated.ts untouched).
 *
...
const OUT = "src/content/questions.generated.3.ts";
...
for (const file of readdirSync(SCRATCH).filter((f) => f.startsWith("gen3-") && f.endsWith(".json")).sort()) {
  let parsed: unknown;
  try {
...
console.log(`Wave-3 new items: ${all.length}  (dropped ${dropped} duplicates)`);
...
if (process.argv.includes("--write")) {
  ...
  const ts = `import type { SeedQuestion } from "./seed-types";\n\n// AUTO-GENERATED by scripts/build-seed-questions-2.mts. Do not edit by hand.\n// Wave 3: more original questions per skill, independently blind-verified.\nexport const GENERATED_QUESTIONS_3: SeedQuestion[] = [\n${body}\n];\n`;
  writeFileSync(OUT, ts);
  console.log(`\nWrote ${all.length} wave-3 questions to ${OUT}`);
}
