/**
 * Blind verification for wave-4 questions (scratchpad/gen4-*.json).
 *  --blind : write blind4-<file>.json (ref/type/stem/options, NO key) + keys4.json
 *  --diff  : compare verdict4-*.json ([{ref, answer}]) to keys4.json
...
const files = () =>
  readdirSync(SCRATCH).filter((f) => f.startsWith("gen4-") && f.endsWith(".json")).sort();
...
if (process.argv.includes("--blind")) {
  ...
  for (const file of files()) {
    ...
    const topic = file.replace(/^gen4-/, "").replace(/\.json$/, "");
    ...
    writeFileSync(join(SCRATCH, `blind4-${topic}.json`), JSON.stringify(blind, null, 2));
  }
  writeFileSync(join(SCRATCH, "keys4.json"), JSON.stringify(keys));
  console.log(`Blinded ${Object.keys(keys).length} wave-4 items across ${files().length} files.`);
}

if (process.argv.includes("--diff")) {
  const keys = JSON.parse(readFileSync(join(SCRATCH, "keys4.json"), "utf8")) as Record<
    string,
    { type: string; correct: unknown }
  >;
  const verdicts: Record<string, unknown> = {};
  for (const f of readdirSync(SCRATCH).filter((f) => f.startsWith("verdict4-") && f.endsWith(".json")))
