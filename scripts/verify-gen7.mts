/**
 * Blind verification for wave-7 questions (scratchpad/gen7-*.json).
 *  --blind : write blind7-<file>.json (ref/type/stem/options, NO key) + keys7.json
 *  --diff  : compare verdict7-*.json ([{ref, answer}]) to keys7.json
...
const files = () =>
  readdirSync(SCRATCH).filter((f) => f.startsWith("gen7-") && f.endsWith(".json")).sort();
...
if (process.argv.includes("--blind")) {
  ...
  for (const file of files()) {
    ...
    const topic = file.replace(/^gen7-/, "").replace(/\.json$/, "");
    ...
    writeFileSync(join(SCRATCH, `blind7-${topic}.json`), JSON.stringify(blind, null, 2));
  }
  writeFileSync(join(SCRATCH, "keys7.json"), JSON.stringify(keys));
  console.log(`Blinded ${Object.keys(keys).length} wave-7 items across ${files().length} files.`);
}

if (process.argv.includes("--diff")) {
  const keys = JSON.parse(readFileSync(join(SCRATCH, "keys7.json"), "utf8")) as Record<
    string,
    { type: string; correct: unknown }
  >;
  const verdicts: Record<string, unknown> = {};
  for (const f of readdirSync(SCRATCH).filter((f) => f.startsWith("verdict7-") && f.endsWith(".json")))
