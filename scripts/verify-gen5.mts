/**
 * Blind verification for wave-5 questions (scratchpad/gen5-*.json).
 *  --blind : write blind5-<file>.json (ref/type/stem/options, NO key) + keys5.json
 *  --diff  : compare verdict5-*.json ([{ref, answer}]) to keys5.json
...
const files = () =>
  readdirSync(SCRATCH).filter((f) => f.startsWith("gen5-") && f.endsWith(".json")).sort();
...
if (process.argv.includes("--blind")) {
  ...
  for (const file of files()) {
    ...
    const topic = file.replace(/^gen5-/, "").replace(/\.json$/, "");
    ...
    writeFileSync(join(SCRATCH, `blind5-${topic}.json`), JSON.stringify(blind, null, 2));
  }
  writeFileSync(join(SCRATCH, "keys5.json"), JSON.stringify(keys));
  console.log(`Blinded ${Object.keys(keys).length} wave-5 items across ${files().length} files.`);
}

if (process.argv.includes("--diff")) {
  const keys = JSON.parse(readFileSync(join(SCRATCH, "keys5.json"), "utf8")) as Record<
    string,
    { type: string; correct: unknown }
  >;
  const verdicts: Record<string, unknown> = {};
  for (const f of readdirSync(SCRATCH).filter((f) => f.startsWith("verdict5-") && f.endsWith(".json")))
