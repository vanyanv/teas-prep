/**
 * Wave-5 question integration. Reads scratchpad/gen5-*.json, validates, drops
 * any stem that duplicates an existing question (the committed bank) or another
 * wave-5 item, shuffles options, and emits src/content/questions.generated.5.ts
 * as GENERATED_QUESTIONS_5 (additive — leaves questions.generated.ts untouched).
 *
...
const OUT = "src/content/questions.generated.5.ts";
...
for (const file of readdirSync(SCRATCH).filter((f) => f.startsWith("gen5-") && f.endsWith(".json")).sort()) {
  let parsed: unknown;
  try {
...
console.log(`Wave-5 new items: ${all.length}  (dropped ${dropped} duplicates)`);
...
if (process.argv.includes("--write")) {
  ...
  const ts = `import type { SeedQuestion } from "./seed-types";\n\n// AUTO-GENERATED by scripts/build-seed-questions-2.mts. Do not edit by hand.\n// Wave 5: more original questions per skill, independently blind-verified.\nexport const GENERATED_QUESTIONS_5: SeedQuestion[] = [\n${body}\n];\n`;
  writeFileSync(OUT, ts);
  console.log(`\nWrote ${all.length} wave-5 questions to ${OUT}`);
}
